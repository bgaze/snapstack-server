...
// Disambiguate same-second captures: "<stamp>", then "<stamp> (2)", "(3)"...
async function uniqueStem(stamp, ext) {
  if (!(await fileExists(path.join(config.dir, `${stamp}.${ext}`)))) return stamp;
  for (let n = 2; ; n += 1) {
    const stem = `${stamp} (${n})`;
    if (!(await fileExists(path.join(config.dir, `${stem}.${ext}`)))) return stem;
  }
}
...
export async function write(bytes, mediaType, meta = {}) {
  ...
  const ts = Date.now();
  const stem = await uniqueStem(localStamp(new Date(ts)), ext);
  ...
  return { stem, name: `${stem}.${ext}`, ext, mediaType: MIME_BY_EXT[ext], imagePath, jsonPath, metadata };
}
...
export async function list() {
  ...
  // Chronological order by write time. The human-readable names are NOT
  // lexicographically chronological (the " (2)" collision suffix sorts before
  // ".ext"), so we sort by mtime, with the name as a stable tiebreaker.
  entries.sort(
