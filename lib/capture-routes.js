...
export async function handleCaptureRequest(req, res, pathname) {
  ...
  // --- capture policy (server-owned, shared across the user's browsers) ----
  if (pathname === '/config' && req.method === 'GET') {
    try {
      return sendJson(res, 200, await settings.readConfig());
    } catch (e) {
      return sendJson(res, 500, { ok: false, error: e.message });
    }
  }
  ...
  if (pathname === '/config' && req.method === 'POST') {
    return handleConfigWrite(req, res);
  }
...
// Persist the capture policy. The body is a small JSON object of validated
// scalars (no filesystem path, no image bytes) — much lower stakes than /push —
// but still write-guarded by the same Host + CORS checks as every route here.
async function handleConfigWrite(req, res) {
  ...
  try {
    return sendJson(res, 200, await settings.writeConfig(input));
  } catch (e) {
    if (e.code === 'INVALID_CONFIG') {
      return sendJson(res, 400, { ok: false, error: 'invalid config', issues: e.issues });
    }
    return sendJson(res, 500, { ok: false, error: e.message });
  }
}
